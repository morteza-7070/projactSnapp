...
<form method="POST" action="{{route('Order.store')}}"  id="createOrder">
    @csrf
    ...
    <input type="number" name="quantity" placeholder="enter quantity" class="quantity"><br>
    ...
    <select name="discount_id" class="discount">
        @foreach($discounts as $discount)
            <option value="{{$discount->id}}" class="active">{{$discount->percentage}}</option>
        @endforeach
    </select><br>
